...
extern struct DamnInode * inodes;
struct DirEntry rt_entry = {{'/', '\0'}, 0};


struct OpenFile {
	uint32_t o_fileid;	// file id
	struct DirEntry *o_file;	// mapped descriptor for open file
	   ...
	   void **pg_store, int *perm_store)
{
	...
	if (req->req_omode & O_MKDIR) {
		if ((r = file_create(path, &f, FTYPE_DIR) < 0))
			return r;
	}
	...
	if (req->req_omode & O_CREAT) {
		if ((r = file_create(path, &f, FTYPE_REG)) < 0) {
...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if((r = openfile_lookup(envid, req->req_fileid, &o)) < 0)
		return r;
	if((r = file_read(o->o_file, (void *)ret->ret_buf,
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	if((r = openfile_lookup(envid, req->req_fileid, &o)) < 0)
		return r;
	if((r = file_write(o->o_file, (void *)req->req_buf,
		req->req_n, o->o_fd->fd_offset)) >= 0)
		o->o_fd->fd_offset += r;
	return r;
...
serve_stat(envid_t envid, union Fsipc *ipc)
{
	...
	if (o->o_file == NULL) {
		...
		inode = &inodes[o->o_file->in_ind];
		strcpy(ret->ret_name, o->o_file->f_name);
	}
	ret->ret_size = inode->size;
	ret->ret_isdir = (inode->type == FTYPE_DIR);
	return 0;
}
...
// Mkdir
int
serve_mkdir(envid_t envid, struct Fsreq_mkdir *req)
{
	char path[MAXPATHLEN];

	if (debug)
		cprintf("serve_mkdir %08x %s\n", envid, req->req_path);
	...
	// Create new dir
	return file_create(path, 0, FTYPE_DIR);
}
...
fshandler handlers[] = {
	...
	[FSREQ_MKDIR] =	(fshandler)serve_mkdir,
